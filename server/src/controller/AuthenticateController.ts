...
export class AuthenticationsController {
    async Login(req, res) {
        ...
        console.log(`Login info emailAddress : ${emailAddress}`, "Login Check", 0);

        if (emailAddress == '') {
            console.log('Username should not be empty');
            errorMessage = await common.GetErrorMessage(400, `emailAddress should not be empty!!`);
        ...
        try {
            ...
            let users = await userService.getUserDetail(emailAddress);
            let userTypeResult: any = await userTypeService.loadById(users[0]?.usertypeid,'usertypeid');

            if (users && users.length > 1 && userTypeResult) {
                let retryData: any = await userService.CheckSuspended(users[0].userid);


                if (retryData.is_user_suspended == true) {
                    console.log(`emailAddress: ${emailAddress}`, "Login attempts exceeded", 0);
                ...
                if ((users[0].password == common.HashPassword(users[0].salt, password)) && retryData.is_user_suspended == false) {
                    ...
                    let obj: any = await this.UserLogin(context, users, userTypeResult);
            ...
            console.log(`emailAddress : ${emailAddress}`, "Unauthorized", 0);
            ...
            console.log(`emailAddress : ${emailAddress}, Error : ${error.message}`, "Unauthorized", 0);
    ...
    async Authorize(req, res, next) {
        ...
        try {
            if (common.isGuid(authKey)) {
                ...
                if (userDetail) {
                    let roles: any = userDetail[0].roles;
                    req.UserId = loggedInUser[0].userid;
                    req.UserGuid = user.guid;
                    req.RoleId = roles[0].roleid;
                    req.RoleCode = roles[0].userroleid;
                    req.AuthKey = authKey;
